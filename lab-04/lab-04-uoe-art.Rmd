...
```{marginfigure}
See the sidebar [here](https://collections.ed.ac.uk/art) and note that there are 2986 pieces in the art collection we're collecting data on.
```
...
```{marginfigure}
**Reminder:** The collection has 2986 pieces in total.
```
...
Click through the first few of the pages in the art collection and observe their URLs to confirm the following pattern:
    ...
    [sometext]offset=2980  # Pieces 2981-2986
...
```{marginfigure}
**Hint:** Make sure to remove `eval=FALSE` from the code chunk label once you have changed the code to make sure you avoid issues when you knit.
```
