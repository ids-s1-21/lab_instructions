...
```{marginfigure}
See the sidebar [here](https://collections.ed.ac.uk/art) and note that there are 2983 pieces in the art collection we're collecting data on.
```
...
-   **Task 1:** Watch [this week's videos](https://idsed.digital/week06/)!
...
We will start off by scraping data on the first 10 pieces in the collection from [the first page of the collection listing](https://collections.ed.ac.uk/art/search/*:*/Collection:%22edinburgh+college+of+art%7C%7C%7CEdinburgh+College+of+Art%22?offset=0).
...
We make use of the SelectorGadget to identify the tags for the relevant elements:

```{r iteminfo-h3a, fig.margin = TRUE, echo = FALSE}
knitr::include_graphics("img/iteminfo-new.gif")
```

```{r}
page %>%
  html_elements(".iteminfo") %>%
  html_element(".record-title a")
```

```{marginfigure}
**Erratum:** In the video, Mine refers to the functions `html_node` and `html_nodes`.  After that video was made, version 1 of the rvest package was released, which renamed these functions to `html_element` and `html_elements` respectively.  Though the old functions still work, they are now "deprecated", and so we use the new names here.
```
...
The same elements that contain the text for the titles also contains information on the links to individual art piece pages for each title.
...
And this is how the text would look on a webpage: [Search on Google](https://www.google.com).
...
page %>%
  ...
  html_element(".record-title a") %>%         # as before
...
```{marginfigure}
**Reminder:** The collection has 2983 pieces in total.
```

That means we give develop a list of URLs (of pages that each have 10 art pieces), and write some code that applies the `scrape_page()` function to each page, and combines the resulting data frames from each page into a single data frame with 2983 rows and 3 columns.
## List of URLs
...
Click through the first few of the pages in the art collection and observe their URLs to confirm the following pattern:
    ...
    [sometext]offset=2970  # Pieces 2971-2980
    [sometext]offset=2980  # Pieces 2981-2983

We can construct these URLs in R by pasting together two pieces: (1) a common (`root`) text for the beginning of the URL, and (2) numbers starting at 0, increasing by 10, all the way up to 2980.
...
```{marginfigure}
**Hint:** Remember escaping special characters from this weeks lecture? You'll need to use that trick again.
```
...
```{marginfigure}
**Hint:** `str_subset()` can be helpful here. Y
