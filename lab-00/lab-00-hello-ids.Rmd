...
The primary goal in all of this is to form your teams for the class, check your all setup for the course, and to have a practice of with some of the core technology we will be using.
If you do not get through all of the Lab work today, this is not an issue.

```{marginfigure}
**Example:** You turn up to the workshop and after introducing yourself, your tutor will tell you that you are allocated to group 12. After a brief introduction you will then be asked to form a smaller team of around 3 students with other students in your same group. After forming them your tutor will come round and tell you that you are now "group12_2". You can then use this to find your workshop repository later in the workshop.
```

Each workshop session will start with a brief introduction in the "big group" and then you will form your teams.
Who you can form a team with will depend on your pre-allocated group number.
These groups have been formed by the timetable unit for track-and-trace purposes (hence why we need you to turn up to the right room and time).
These groups are a little big for the purposes of the class (they are around 12 students), so we will ask you to form smaller subteams (\~3 students).
Once formed, we will allocate you a number.

If you are online for this session, you will be told to go to your group's breakout room to find other online teammates.
When in a breakout room everyone should turn on their audio and video (if possible).
...
**Note:** Each participant in the team needs to have completed the "first time setup" to be able to do the rest of the workshop.
    Team members who have already got themselves set up can move on and include the remaining members once they are ready.
```
...
3.  Before the session, you should have completed the [Getting to know you survey](https://forms.office.com/Pages/ResponsePage.aspx?id=sAafLmkWiUWHiRCgaTTcYdffF13VDSBAsmaFgj3BcYtUREVZQUVSWjY2UlZDTVdMWjM3NzI3VzVQUi4u) and set up accounts on:
    ...
    If you encounter any issues you can ask for help by raising your hand (in person) or by pressing the **Ask for help** button (within Zoom).

We're now going to set up a RStudio Cloud project.
...
12. **Commit** your changes by pressing commit and ticking all the boxes on the left, type your message in the box, and then press **commit**.
    Now **Pull** again (in case anyone has made changes since the last time you pulled---they shouldn't have if you have been taking turns!), and push, using the Git tab at the top-right.
